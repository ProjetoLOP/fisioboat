...
// Reset do histórico quando um squat é detectado
...
async function prepareDetector() {
  ...
  try {
    ...
    const modelPath = '/Assets/scripts/libs/models/singlepose-lightning/model.json';
  ...
  console.log("[prepareDetector] Iniciando warmup sintético (sem vídeo)...");

  // 🔥 Criar uma imagem fake (tensor preto) para "aquecer" o modelo
  ...
  try {
    ...
    console.warn("[prepareDetector] Erro no warmup sintético:", err);
  }
  ...
  console.log("[prepareDetector] Detector pronto e aquecido (sem vídeo).");
}
