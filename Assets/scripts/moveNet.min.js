...
// Função para calcular profundidade e disparar evento (se for válida)
function computeDepthAndDispatch() {
  const depth = computeDepth();

  // Verifica se a profundidade é válida (não negativa)
  if (depth < 0.05) {
    // console.log("Movimento desconsiderado: profundidade negativa.", { depth });
    return;
  }
...
function processPose(pose) {
  ...
  // Fase de subida
    ...
    if (squatFrames >= stabilityFrames) {
      ...
      const detail = { depth, instant: new Date().toISOString() };
      console.log(detail);
